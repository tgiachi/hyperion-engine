...
namespace util
{
  ...
  class Matrix
  {
   public:
    ...
    const shape_type& get_shape() const noexcept
    {
      return shape_;
    }
  ...
  class Array2D
  {
   public:
    ...
    Array2D(const shape_type& shape, const T& fill_value) : shape_(shape), data_(get_size_from_shape(shape), fill_value)
    {
    }
    auto begin() noexcept
    {
    ...
    size_type get_width() const noexcept
    {
      return shape_.at(0);
    }
    size_type get_height() const noexcept
    {
      return shape_.at(1);
    }
   ...
   private:
    ...
    size_t get_index(const index_type& index) const noexcept
    {
      return index.at(1) * shape_.at(0) + index.at(0);
    }
    size_t check_range(const index_type& index) const
    {
      if (!in_bounds(index))
      {
        throw std::out_of_range(
            std::string("Out of bounds lookup ") + array_as_string(index) + " on array of shape " + array_as_string(shape_) +
            ".");
      }
      return get_index(index);
    }
